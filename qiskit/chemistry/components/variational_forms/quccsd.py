...
class QUCCSD(VariationalForm):
    ...
    def __init__(self, num_qubits: int,
        ...
        self._single_excitations, self._double_excitations = \
            QUCCSD.compute_excitation_lists([self._num_alpha, self._num_beta], self._num_orbitals,
        ...
        if self._method_doubles == 'succ_full':
            ...
            self._single_excitations, self._double_excitations = \
                QUCCSD.compute_excitation_lists([self._num_alpha, self._num_beta],
                                               self._num_orbitals,
                                               active_occupied, active_unoccupied,
                                               entangled_occ, entangled_unocc,
                                               same_spin_doubles=self.same_spin_doubles,
        ...
        if self.uccd_singlet:
            ...
            self._double_excitations_grouped = \
                QUCCSD.compute_excitation_lists_singlet(self._double_excitations, num_orbitals)
            ...
            order_hopping_op = QUCCSD.order_labels_for_hopping_ops(self._double_excitations,
                                                                  self._double_excitations_grouped)
    ...
    def _build_hopping_operators(self):
        ...
        results = parallel_map(QUCCSD._build_hopping_operator,
                               self._single_excitations + self._double_excitations,
    ...
    def manage_hopping_operators(self):
        """
        Triggers the adaptive behavior of this QUCCSD instance.
    ...
    def construct_circuit(self, parameters, q=None):
        ...
        results = parallel_map(QUCCSD._construct_circuit_for_one_excited_operator,
                               list_excitation_operators,
    ...
    def compute_excitation_lists_singlet(double_exc, num_orbitals):
        ...
        """
        de_groups = QUCCSD.group_excitations_if_same_ao(double_exc, num_orbitals)
    ...
    def same_ao_double_excitation_block_spin(de_1, de_2, num_orbitals):
        ...
        singlet QUCCSD unlike in usual QUCCSD where every excitation is controlled by independent
        parameter.
    ...
    def group_excitations(list_de, num_orbitals):
        ...
        because those excitations are controlled by the same parameter in full singlet QUCCSD
        unlike in usual QUCCSD where every excitation has its own parameter.
        ...
        counter = 0
        for de2 in list_de:
            if QUCCSD.same_ao_double_excitation_block_spin(de1, de2, num_orbitals) == 1:
                counter += 1
                if counter == 1:
    ...
    def group_excitations_if_same_ao(list_de, num_orbitals):
        ...
        num_orbitals, which excitations involve the same spatial orbitals for full singlet QUCCSD.
        ...
        """
        list_groups = []
        list_same_ao_group, list_de_temp = QUCCSD.group_excitations(list_de, num_orbitals)
        list_groups.append(list_same_ao_group)
        while len(list_de_temp) != 0:
            list_same_ao_group, list_de_temp = QUCCSD.group_excitations(list_de_temp, num_orbitals)
            list_groups.append(list_same_ao_group)
    ...
    def order_labels_for_hopping_ops(double_exc, gde):
        ...
        Args:
            ...
            gde (list of lists): list of grouped excitations for full singlet QUCCSD
